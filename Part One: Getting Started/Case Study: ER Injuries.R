library(shiny) # for the app development
library(vroom) # for fast file reading
library(tidyverse) # for general data analysis outside of our shiny application

# The Data
# Records of all accidents seen in a representative sample of hospitals in the United 
# States. Focus on the data from 2017
...
############################################################################
## Some data exploration of the underlying data
### A product 649, "toilets", how many injuries associated with this product
selected <- injuries %>% filter(prod_code == 649)
nrow(selected)

## basic summaries on the product 649: looking at the location, body part and diagnosis
...
summary_649 <- selected %>%
  count(age, sex, wt = weight)
summary_649

###
# A plot would make the trend more obvious
summary_649 %>%
...
###
# Control by comparing the number of people injured with the total population 
# and calculating the injury rate. Here we use rate per 10000
summary_update <- selected %>%
  count(age, sex, wt = weight) %>%
...
###
# Pull 10 random sample narratives to check the narrative
selected %>%
  sample_n(10) %>%
  pull(narrative)
